...
import {
  extractZipped,
  getRemoteFile,
} from "../../src/synopsys-action/download-utility";
import {
  cleanupTempDir,
  createTempDir,
} from "../../src/synopsys-action/utility";
...
test("Test getRemoteFile", () => {
  ...
  getRemoteFile(tempPath, "http://synopsys.bridge.com/bridge_macOs_1.zip").then(
    (data) => {
...
test("Test getRemoteFile linux", () => {
  ...
  getRemoteFile(tempPath, "http://synopsys.bridge.com/bridge_linux_1.zip").then(
    (data) => {
      expect(data.fileName).toContain("bridge");
    }
  );
...
test("Test getRemoteFile windows", () => {
  ...
  getRemoteFile(tempPath, "http://synopsys.bridge.com/bridge_win_1.zip").then(
    (data) => {
      expect(data.fileName).toContain("bridge");
    }
  );
