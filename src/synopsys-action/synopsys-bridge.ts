...
export class SynopsysBridge {
  ...
  constructor() {
    ...
    this.bridgeUrlPattern = this.bridgeArtifactoryURL.concat(
      "/$version/ci-package-$version-$platform.zip "
    );
  }
  ...
  private async checkIfSynopsysBridgeExists(): Promise<boolean> {
    ...
    if (osName === "win32") {
      this.bridgeExecutablePath = await tryGetExecutablePath(
        synopsysBridgePath.concat("\\bridge"),
        [".exe"]
      );
    } else {
      this.bridgeExecutablePath = await tryGetExecutablePath(
    ...
    if (this.bridgeExecutablePath) {
      ...
      info(
        "Bridge executable could not be found at ".concat(synopsysBridgePath)
      );
    }
  ...
  async executeBridgeCommand(
    ...
    if (await this.checkIfSynopsysBridgeExists()) {
      ...
      if (osName === "darwin" || osName === "linux" || osName === "win32") {
        ...
        try {
          return await exec(
            this.bridgeExecutablePath.concat(" ", bridgeCommand),
            [],
            exectOp
          );
        } catch (error) {
  ...
  async downloadBridge(tempDir: string): Promise<void> {
    try {
      ...
      if (inputs.BRIDGE_DOWNLOAD_VERSION) {
        ...
        info(
          "Provided Bridge download url is - ".concat(
      ...
      const downloadResponse: DownloadFileResponse = await getRemoteFile(
        tempDir,
        bridgeUrl
      );
  ...
  async prepareCommand(tempDir: string): Promise<string> {
    try {
      ...
      if (
        inputs.POLARIS_SERVER_URL == null &&
      ...
      if (formattedCommand.length === 0) {
        return Promise.reject(
          new Error("Mandatory fields are missing for given scan[s]")
        );
      }
