...
export class SynopsysToolsParameter {
  ...
  getFormattedCommandForPolaris(): string {
    ...
    if (
      inputs.POLARIS_ASSESSMENT_TYPES != null &&
    ...
    for (const assessmentType of assessmentTypes) {
      if (
        !Object.values(PolarisAssessmentType).includes(
          assessmentType as PolarisAssessmentType
        )
      ) {
        throw new Error("Provided Assessment type not found");
      } else {
        assessmentTypeEnums.push(
          PolarisAssessmentType[
            assessmentType as keyof typeof PolarisAssessmentType
          ]
        );
      }
    }
  ...
  getFormattedCommandForCoverity(): string {
    ...
    if (inputs.COVERITY_INSTALL_DIRECTORY) {
      ...
      if (osName === "win32") {
        validateCoverityInstallDirectoryParam(
          inputs.COVERITY_INSTALL_DIRECTORY
        );
      }
